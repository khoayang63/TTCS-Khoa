...
def draw_professional_box(image, x1, y1, x2, y2, label_text, color, font_scale=0.5, thickness=2):
    ...
    # Tính toán vị trí nền chữ (Xử lý trường hợp vật thể sát mép trên ảnh)
    txt_box_y1 = y1 - text_h - baseline - 5
    if txt_box_y1 < 0:
        txt_box_y1 = y1 + 1

    # Vẽ nền chữ và chèn nhãn (Màu trắng trên nền màu của Class)
...
def predict_image(image_path, model, device="cuda", threshold=0.6, iou_threshold=0.3, agnostic_nms=False):
    ...
    # 1. Tiền xử lý ảnh (Resize, chuẩn hóa và chuyển sang Tensor)
    original_image = cv2.imread(image_path)
    img_h, img_w, _ = original_image.shape
    image = cv2.resize(cv2.cvtColor(original_image, cv2.COLOR_BGR2RGB), (config.IMAGE_SIZE, config.IMAGE_SIZE))
    image = (image / 255.0 - [0.485, 0.456, 0.406]) / [0.229, 0.224, 0.225]
    x = torch.from_numpy(image).permute(2, 0, 1).float().unsqueeze(0).to(device)

    # 2. Dự đoán và giải mã tọa độ từ 3 quy mô (Scales)
    with torch.no_grad():
        predictions = model(x)
    bboxes = []
    for i in range(3):
        S = predictions[i].shape[2]
        anchor = torch.tensor([*config.ANCHORS[i]]).to(device) * S
        bboxes += cells_to_bboxes(predictions[i], anchor, S=S, is_preds=True, output_format='xyxy')[0]

    # 3. Lọc dự đoán theo ngưỡng Score và kích thước vật thể
    boxes = torch.tensor(bboxes).to(device)
    ...
    # 4. Hậu xử lý bằng thuật toán NMS (Batched hoặc Agnostic)
    if agnostic_nms:
        keep = ops.nms(coords, scores, iou_threshold)
    else:
        keep = ops.batched_nms(coords, scores, class_preds, iou_threshold)
    
    coords, scores, class_preds = coords[keep[:100]], scores[keep[:100]], class_preds[keep[:100]]

    # 5. Hiển thị kết quả chuyên nghiệp
    np.random.seed(42)
    ...
    for i in range(len(coords)):
        ...
        x1, y1 = max(0, int(box[0] * img_w)), max(0, int(box[1] * img_h))
        x2, y2 = min(img_w, int(box[2] * img_w)), min(img_h, int(box[3] * img_h))
        
        draw_professional_box(original_image, x1, y1, x2, y2, f"{VOC_CLASSES[cid]}: {sc:.2f}", colors[cid])

    cv2.imshow("YOLOv3 Inference", original_image)
...
def main():
    ...
    model.load_state_dict(torch.load('weights/best_weight.pt'))
